...
export default async function EditSoilParametersModal({params}:{params: Promise<{id: string}>}) {
  ...
  .from('soils')
  .select("start_depth, end_depth, y_moist, y_sat, n_value, soil, soil_name, soil_type, soil_profile_id, test_type, qca, qc, kc, a")
  .eq('id', id)
  .single()

  if (error) {
    return (
      <Modal title="Error - Edit Soil Parameters">
