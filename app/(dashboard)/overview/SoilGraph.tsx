'use client'
import { ToverviewSoilSchema } from "@/schemas/soilSchemas";
import { ToverviewSoilProfileSchema } from "@/schemas/soilProfileSchemas";
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts"

export function SoilGraph({ 
  profileSoils, profile, pileDiameter}: { profileSoils: ToverviewSoilSchema[], profile: ToverviewSoilProfileSchema,  pileDiameter: 60 | 100 }) {
  
  const filteredSoils = profileSoils.filter(soil => soil.end_depth <= profile.effective_pile_length)
  
  const shaftCapacityKey = pileDiameter === 60 ? "shaft_capacity60" : "shaft_capacity100"
  const bearingCapacityKey = pileDiameter === 60 ? "bearing_capacity60" : "bearing_capacity100"
  
  return (
    <div className="w-full h-100">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={filteredSoils} width={600} height={300} margin={{ top: 20, right: 30, left: 40, bottom: 60 }}>
          <XAxis dataKey="start_depth" name="Depth (m)" label={{ value: 'Depth / m', position: 'insideBottom', offset: -5 }} domain={['dataMin', 'dataMax']}/>
          <YAxis name="Capacity" label={{ value: 'Capacity / kN', angle: -90, position: 'insideLeft' }} domain={[0, 'dataMax']}/>
          <Line type="monotone" dataKey={shaftCapacityKey} stroke="#82ca9d" name="Shaft Capacity" activeDot={{ r: 6 }} />
          <Line type="monotone" dataKey={bearingCapacityKey} stroke="#ff0000" name="Bearing Capacity" activeDot={{ r: 6 }} /> 
          <Tooltip />
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
}