...
export const Sidebar = () => {
  ...
  return (
    <>
      ...
      <aside className={`bg-sidebar flex flex-col h-full shrink-0 border-r border-sidebar-border p-2 shadow-inner overflow-y-auto overflow-x-clip scrollbar-thin scrollbar-thumb-rounded scrollbar-track-rounded scrollbar-thumb-slate-400 scrollbar-track-slate-200 scrollbar-hover:scrollbar-thumb-slate-500 scrollbar-active:scrollbar-thumb-slate-500 fixed left-0 top-0 z-50 w-[260px] sm:static sm:translate-x-0 ${isResizing ? "" : "transition-transform duration-500 ease-in-out"} ${mobileMenuOpen ? "translate-x-0" : "-translate-x-full"} ${expanded ? (isResizing ? "sm:w-[260px]" : "sm:w-[260px] sm:transition-all sm:duration-500 sm:ease-in-out") : (isResizing ? "sm:w-[70px]" : "sm:w-[70px] sm:transition-all sm:duration-500 sm:ease-in-out")}`}>
        {expanded ? (
                  ...
                  {section.items.map((item) => (
                    <li key={item.href} className='my-3 animate-in fade-in slide-in-from-top-8 duration-500'>
                      <Button asChild variant="ghost" className='hover:bg-sidebar-foreground/7 dark:hover:bg-sidebar-foreground/7'>
                        <Link prefetch={true} scroll={false} href={item.href as Route} className="flex gap-3 w-full justify-start"  onClick={() => setMobileMenuOpen(false)}><item.icon className='size-6'/>{item.text}</Link>
                      </Button>
                    </li>
                  ))}
                </Fragment>
