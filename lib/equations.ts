...
export async function calculateResultsForSoils (soil: TsoilSchema, waterDepth: number): Promise<Partial<TsoilSchema>> {
...
//Fine Soil Algorithm
export async function calculateResultsForFineSoil (soil: TsoilSchema): Promise<Partial<TsoilSchema>> {
  const h = roundToTwoDecimals(soil.endDepth - soil.startDepth)
